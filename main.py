...
class getCurrentStatus:
   ...
   def __init__(self,InputPin,processOn,processOff):
         ...
         if(flag == 0 and SignalStatus==1):
            ...
            #check if pattern matches and mark production count = count+1 in production database
            if(process=="cycleON"):
               print(tempProductionArray)
               print(productionArray)
               tempProductionArray=[] 
               tempProductionArray.append(process)
               self.liveSignal("Cycle On")
            elif(process=="m30ON"):
               tempProductionArray.append(process)
            else:
              pass   
            ...
            #check if alarm has occured if so render alarm screen
            if process=="alarmON":
              self.liveSignal("Alarm ON")
            elif process=="EmergencyON":
              self.liveSignal("Emergency On")
         ...
         if(flag == 1 and SignalStatus == 0):
            ...
            if(productionArray==tempProductionArray):
              ...
              #except:
               ...
               if(process=="m30OFF"):
                  self.machineHold("Hold")
               elif process=="EmergencyOFF":
                  self.liveSignal("Machine Idle")
               elif process=="cycleOFF":
                  self.liveSignal("Machine Idle")  
               else:
                 pass
   ...
   def liveSignal(self,signal):
          if(signal=="Alarm ON"):
             res=req.post(localServer,json.dumps({"State":"Hold"}),headers=header,timeout=2)
             print(res.status_code)
          res=req.post("127.0.0.1:3000/liveSignals",json.dumps({"liveSignal":signal}),headers=header,timeout=2)
          if(res):
             print("success")
